  ...
  if (blank) {
    ...
    if (managerIdList.trim() === '') {
      ...
      for (const word of splitWord) {
        if (word.trim().toLowerCase() === managerId.toLowerCase()) {
      ...
      arrList.push(managerId);
      const clearList = checkDuplicateArrWord(arrList);
...
const removeChatManagerId = () => {
  ...
  if (blank) {
    ...
    const clearList = checkDuplicateArrWord(arrList2);
